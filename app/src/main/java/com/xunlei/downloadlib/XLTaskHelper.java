...
//        return currentFileInfo;
    ...
    public synchronized void deleteTaskFile(long j, final String str) {
        stopTask(j);
    ...
    public synchronized void deleteTaskDir(long j, final String str) {
        stopTask(j);
        new Handler(Daemon.looper()).post(new Runnable() {
            public void run() {
                try {
    ...
    public long addBt(String str, String str2, boolean z, HashSet<Integer> hashSet) {
        ...
        if (z) {
            ...
            ordinal = XLConstant.XLCreateTaskMode.NEW_TASK.ordinal();
        }
        btTaskParam.setCreateMode(ordinal);
        ...
        for (TorrentFileInfo torrentFileInfo : torrentInfo.mSubFileInfo) {
            if (hashSet.contains(Integer.valueOf(torrentFileInfo.mFileIndex))) {
